    ...
    <div class="ticket-modal-info">
      ...
      <div class="bottom">
        ...
        <div class="ticket-item-wrapper">
          <p class="uppercase d-inline description-text medium">
            {{ $t("events.modal.info_2") }}
          </p>
          <div class="d-inline-flex justify-content-start align-items-center">
            <i class="fa-solid fa-minus" @click="decrementTicketRegular"></i>
            <p class="uppercase d-inline description-title">
              {{ ticketRegular }}
            </p>
            <i class="fa-solid fa-plus" @click="incrementTicketRegular"></i>
          </div>
        </div>
        <div class="ticket-item-wrapper">
          <p class="uppercase d-inline description-text medium">
          ...
          <div class="d-inline-flex justify-content-start align-items-center">
            <i class="fa-solid fa-minus" @click="decrementTicketDiscounted"></i>
            <p class="uppercase d-inline description-title">
              {{ ticketDiscounted }}
            </p>
            <i class="fa-solid fa-plus" @click="incrementTicketDiscounted"></i>
          </div>
        </div>
        <div
          class="ticket-item-total d-flex justify-content-center align-items-center"
        >
          <p class="lowercase description-text text-center black">
            {{ $t("events.modal.info_4") }}:
            <span class="description-text font-bold d-inline big"
              >{{ total }} €</span
            >
          </p>
        </div>
        <form class="form">
          ...
          <div class="input-wrapper big">
            <input
              type="num"
              maxlength="19"
              :placeholder="$t('events.modal.form.card_number')"
            />
          </div>
          <div
          ...
          >
            <div class="input-wrapper small">
              <input
                type="num"
                maxlength="2"
                :placeholder="$t('events.modal.form.month')"
              />
            </div>
            <div class="input-wrapper small">
              <input
                type="num"
                maxlength="2"
                :placeholder="$t('events.modal.form.year')"
              />
            </div>
            <div class="input-wrapper small">
              <input
                type="password"
                maxlength="3"
                :placeholder="$t('events.modal.form.cvc')"
              />
            </div>
          </div>
          <div
            class="ticket-modal-submit-wrapper d-flex justify-content-center"
          >
            <button
              type="submit"
              class="ticket-modal-submit description-title capitalize d-inline"
            >
